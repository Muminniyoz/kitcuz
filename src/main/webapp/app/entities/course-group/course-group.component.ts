...
export class CourseGroupComponent implements OnInit, OnDestroy {
  ...
  loadAll(): void {
    this.courseGroupService
      ...
      .subscribe((res: HttpResponse<ICourseGroup[]>) => this.paginateCourseGroups(res.body, res.headers));
  }
  ...
  ngOnInit(): void {
    this.loadAll();
    this.registerChangeInCourseGroups();
  }
  ...
  registerChangeInCourseGroups(): void {
    this.eventSubscriber = this.eventManager.subscribe('courseGroupListModification', () => this.reset());
  }
  ...
  protected paginateCourseGroups(data: ICourseGroup[] | null, headers: HttpHeaders): void {
    const headersLink = headers.get('link');
    this.links = this.parseLinks.parse(headersLink ? headersLink : '');
    if (data) {
      for (let i = 0; i < data.length; i++) {
        this.courseGroups.push(data[i]);
      }
    }
  }
}
